...
function installProgressReset() {
  ...
  button.setAttribute('aria-label', 'ベストスコア、ランク、ミッション、鬼攻略、鬼神攻略、家具図鑑、ステージ進行を初期化');

  row.append(label, button);

  const note = document.createElement('p');
  note.className = 'tcard-p';
  note.textContent = 'ベスト・ランク・ミッション・鬼攻略・鬼神攻略・家具図鑑・ステージ進行を初期化します。サウンド・音量・視点感度・Y軸反転の設定は残ります。';

  card.append(row, note);

  button.addEventListener('click', () => {
    const ok = window.confirm('ベストスコア、ランク、ミッション、鬼攻略、鬼神攻略、家具図鑑、ステージ進行を初期化しますか？\n設定は残ります。');
    if (!ok) return;
...
function installSaveTransfer() {
  ...
  const note = document.createElement('p');
  note.className = 'tcard-p';
  note.textContent = 'ベストスコア・ランク・ミッション・鬼攻略・鬼神攻略・家具図鑑・音量などの設定を1つのファイルにまとめて書き出せます。別のブラウザや端末へ移すときや、初期化前のバックアップに使えます。';
