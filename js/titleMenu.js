// タイトル画面の「あそびかた / 設定 / 特訓 / 鬼神 / チャレンジ / 家具図鑑」をスマホでも快適に扱う。
...
addCloseButton(cardChallenge, btnChallenge);
...
function isOpen() {
  return btnHow?.getAttribute('aria-expanded') === 'true'
    ...
    || btnChallenge?.getAttribute('aria-expanded') === 'true'
    ...
    const active = btnHow?.getAttribute('aria-expanded') === 'true'
      ...
      : (btnConfig?.getAttribute('aria-expanded') === 'true'
        ...
        : (btnTraining?.getAttribute('aria-expanded') === 'true'
          ...
          : (btnKishin?.getAttribute('aria-expanded') === 'true'
            ? cardKishin
            : (btnChallenge?.getAttribute('aria-expanded') === 'true' ? cardChallenge : catalogOverlay))));
...
if (btnHow && btnConfig) {
  ...
  if (btnChallenge) observer.observe(btnChallenge, { attributes: true, attributeFilter: ['aria-expanded'] });
