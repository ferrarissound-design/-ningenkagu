...
test('progress key classifier covers every saved progression family', () => {
  ...
  assert.equal(isProgressKey('ningenkagu.completed6'), true);
  assert.equal(isProgressKey('ningenkagu.challengeClear.noDecoy'), true);
  assert.equal(isProgressKey('ningenkagu.catalogUse.tv'), true);
...
test('clearProgressData removes progression but preserves player settings', () => {
  const storage = new MemoryStorage({
    ...
    'ningenkagu.completed6': '1',
    'ningenkagu.challengeClear.noDecoy': '1',
    'ningenkagu.catalogUse.tv': '3',
    'ningenkagu.muted': '1',
  ...
  assert.equal(storage.getItem('ningenkagu.completed6'), null);
  assert.equal(storage.getItem('ningenkagu.challengeClear.noDecoy'), null);
  assert.equal(storage.getItem('ningenkagu.catalogUse.tv'), null);
