...
test.describe('STAGE 5 理科室', () => {
  ...
  test('理科室クリアでCLASSIC CLEARになり、STAGE 6が解放される', async ({ page }) => {
    await unlockScience(page);
    await page.click('#btnStart');
    await page.waitForFunction(() => window.__ningenkagu.game.state === 'playing');
    ...
    await expect(page.locator('#resultTitle')).toHaveText('CLASSIC CLEAR!');
    await expect(page.locator('#resultNote')).toContainText('STAGE 6');
    ...
    await expect(page.locator('#allClearBadge')).toContainText('クラシック5ステージ制覇済み');
